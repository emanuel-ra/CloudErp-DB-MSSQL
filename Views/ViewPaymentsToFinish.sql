CREATE VIEW ViewPaymentsToFinish AS
SELECT
...
WHERE
    (q.StatusID = 13 AND p.CodeSalesInvoices is null) or (q.StatusID = 21 AND ISNULL(q.Is_packed, 0) = 0 AND ISNULL(q.Is_deliver, 0) = 0)
GROUP BY
    p.InitValid,
    rqp.PaymentId,
    p.Id,
